var messageId = 328880637; //Math.floor(1E9 * Math.random());

function encodePing() {
    function encode(a, b, c, h) {
        a = new Uint8Array(a, c, h);
        for (c = 0; c < h; c++) {
            console.log(`b: ${b} | h: ${h} | c: ${c} | result: ${b >> 8 * (h - 1 - c) & 255} | math: ${8 * (h - 1 - c)}`);
            // console.log('b:', b.toString(2));
            // console.log('o:', (8 * (h - 1 - c) & 255).toString(2));
            a[c] = b >> 8 * (h - 1 - c) & 255;
        }
        console.log(a);
    }
    ...
    // 1569868454913
    messageId++;
    let now = 1569859053692 //Date.now()
