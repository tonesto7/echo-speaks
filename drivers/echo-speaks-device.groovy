...
Map getEchoDevInfo(String cmd, Boolean igndnd=false) {
    if(isCommandTypeAllowed(cmd, false, igndnd)) {
...
Boolean isCommandTypeAllowed(String type, Boolean noLogs=false, Boolean igndnd=false) {
    ...
    if(!igndnd && !(type in ["volumeControl", "alarms", "reminders", "doNotDisturb", "wakeWord", "bluetoothControl", "mediaPlayer"])
...
def playAnnouncement(String msg, volume=null, restoreVolume=null, Boolean parallel=false, Boolean igndnd=false) {
    if(isZone()) {
        if(parallel) {
            parent.relayAnnounceZone(parent.id.toString(), msg, true, igndnd)
        ...
        if(isCommandTypeAllowed("announce", false, igndnd)) {
...
void parallelSpeak(String msg, Boolean igndnd=false) {
    logTrace("parallelSpeak() command received...")
    if (isCommandTypeAllowed("TTS", false, igndnd)) {
        if (!msg) {
            logWarn("No Message sent with parallelSpeak(${fixLg(msg)}) command", true)
        } else {
            if (isZone()) {
                parent.relaySpeakZone(parent.id.toString(), msg, true, igndnd)
...
// capability speechSynthesis
@SuppressWarnings('unused')
void speak(String msg, volume=null, String awsPollyVoiceName = sNULL, Boolean igndnd=false) {
    ...
    if(isCommandTypeAllowed("TTS", false, igndnd)) {
        if(!msg) { logWarn("No Message sent with speak(${fixLg(msg)}) command", true) }
            ...
            if(isZone()) {
                parent.zoneCmdHandler([value: 'speak', jsonData: [zones:[parent.id.toString()], cmd:'speak', message: msg, changeVol: newvol, restoreVol: restvol, delay:0]], true, igndnd)
                String lastMsg = msg ?: "Nothing to Show Here..."
                sendEvent(name: "lastSpeakCmd", value: lastMsg, descriptionText: "Last Text Spoken", display: true, displayed: true, isStateChange:true)
                //String t0 = getDtNow()
                //sendEvent(name: "lastCmdSentDt", value: t0, descriptionText: "Last Command Timestamp: ${t0}", display: false, displayed: false)
